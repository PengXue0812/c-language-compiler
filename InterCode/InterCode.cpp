  ...
  switch (type) {
    ...
    case static_cast<int>(NodeType::DEFINITION){
        if(node_content == "Declare_Statement"){
            ...
            BaseNode *child = node->getChildNode();
            BaseNode *mod_node = node->getPreNode()->getChildNode();
            if(child->getContent() =="Variable"){//只声明不赋值
                ...
                if(name == "Def_array[Const]"){
                    ...
                    if(array_symbol !=nullptr && static_cast<int>(array_symbol->getType()) == static_cast<int>(SymbolType::array)){
                ...
                // }
                    ...
                    if(mod_node->getContent() == "int*_Type"){
                        ...
                        std::string identifier_name = strcpy("*",ccNode->getContent());
                        Symbol * identifier_symbol = area->findSymbolLocally(identifier_name);
                        if(identifier_symbol !=nullptr && static_cast<int>(identifier_symbol->getType()) == static_cast<int>(SymbolType::integer)){
                ...
                if(child->getChildNode()->getContent() == "Def_Identifier"){
                    if(mod_node->getContent() == "int*_Type"){
                      ...
                      std::string pointer_name = child->getChildNode()->getChildNode()->getContent();
                        ...
                        area->addSymbol(var);
                        QuadItem *temp = new QuadItem(var,QuadType::ASSIGN,addr_var);
                        this->quad_list.push_back(temp);
                    }
                    else {
                        ...
                        int var_type = static_cast<int>(var->getType());
                        int symbol_type = static_cast<int>(symbol->getType());
                        if(var_type == 2 && symbol_type == 2){
                            area->setOffset(area->getOffset() + var->getWidth());
                            area->addSymbol(var);
                        }
                        QuadItem* quad;
                        if(isNumber(symbol_type))
                        {
